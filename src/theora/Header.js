...
TheoraJS.namespace('Theora').header = (function() {
        ...
        for (qti = 0; qti < 2; qti += 1) {
            ...
            for (pli = 0; pli < 3; pli += 1) {
                ...
                if (newqr === 0) {
                    ...
                    while (qi < 63) {
                        qrsizes[qti][pli][qri] =
                            reader.nextBits(util.ilog(62 - qi)) + 1;
    ...
    function computeQuantizationMatrix(qti, pli, qi) {
        ...
        for (ci = 0; ci < 64; ci += 1) {
            ...
            qmat[ci] = Math.max(
                qmin,
                Math.min(util.toInt((qscale * bm) / 100) * 4, 4096)
            );
        }
        ...
        decodeIdentificationHeader(packet) {
            ...
            switch (this.pf) {
                case 0:
                     ...
                     */
                    this.nsbs =
                        util.toInt((this.fmbw + 1) / 2) *
                            util.toInt((this.fmbh + 1) / 2) +
                        2 *
                            util.toInt((this.fmbw + 3) / 4) *
                            util.toInt((this.fmbh + 3) / 4);
